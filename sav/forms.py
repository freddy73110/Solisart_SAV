...
class add_evenement_form(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if instal:
            self.fields["installation"].queryset = installation.objects.filter(
                pk=instal.id
            )
        ...
        if user_acces:
            self.fields["installation"].queryset = installation.objects.filter(
                acces__utilisateur=user_acces
            )
            tu = [
                (i.id, str(i))
                for i in installation.objects.filter(
                    acces__utilisateur=user_acces
                ).distinct()
            ]
            tu2 = (
                installation.objects.all()
                .annotate(
                    firstname=Subquery(
                        User.objects.filter(
                            acces__installation__id=OuterRef("pk"),
                            acces__profil_type__name="Propriétaire",
                        ).values("first_name")[:1]
                    ),
                    lastname=Subquery(
                        User.objects.filter(
                ...
                .distinct()
                .values("id", "idsa", "firstname", "lastname")
                .annotate(
                    str=Concat(
                        "idsa", Value(" / "), "firstname", Value(" "), "lastname"
                    )
                )
                .values_list("id", "str")
            )
            ...
            choices = (
                ("Installation(s) liée(s)", tuple(tu)),
...
class noncompliance_form(ModelForm):
    ...
    CHOICES = (
        (
            a.t50_2_code_comp,
            str(a.t50_2_code_comp) + "-" + str(a.t50_37_titre_du_composant),
        )
        for a in B50Composants.objects.db_manager("herakles")
        .all()
        .order_by("t50_2_code_comp")
    )
            ...
            if NC.fichier:
                ...
                for f in NC.fichier.all():
                    file_list += (
                        ...
                        + " "
                        + str(f)
                        + "</a> (<span onclick='deleteNCFile("
                        + str(f.id)
                        + ")'><i class='far fa-trash-alt'></i></span>)</li>"
                    )
                file_list += "</ul>"
        else:
        ...
        self.helper.layout = Layout(
            Custom_Fieldset(
                ...
                Row(
                    ...
                    HTML(
                        '<script>$("#id_noncompliance-list_ref_herakles").on("change", function(){$("#id_noncompliance-ref_herakles").val($(this).val())})</script>'
                    ),
                ...
                Row(
                    Column(
                        FloatingField(
                            "detail", style="height: 100px", css_class="textareaEmoji"
                        )
                    )
                ),
                Row(Column("fichier")),
                Row(Column(HTML(file_list))),
            )
        )
...
class ticket_form(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            htmlnoncompliance = (
                '<div class="form-check form-switch"><input class="form-check-input" type="checkbox" name="Noncompliance" id="InputNoncompliance" checked disabled value="0"><label class="form-check-label" for="flexCheckDefault">Non conformité '
                + str(NC)
                + "</label></div>"
            )
        except:
        ...
        items = (
            probleme.objects.all()
            .values("categorie", "sous_categorie", "id")
            .order_by("categorie", "sous_categorie")
        )
        rows = groupby(items, itemgetter("categorie"))
        di = {
            str(type_probleme(c_title).label): tuple(
                [(i["id"], i["sous_categorie"]) for i in items]
            )
        ...
        rows = groupby(items, itemgetter("categorie"))
        di = {
            str(type_cause(c_title).label): tuple(
                [(i["id"], i["sous_categorie"]) for i in items]
            )
            for c_title, items in rows
        }
        ...
        if instal:
            tu = []
            for i in (
                User.objects.filter(acces__installation=instal)
                .distinct()
        ...
        self.helper.layout = Layout(
            ...
            HTML(emoji_str),
            Row(Column("devis", css_class="col-6"), Column("BL", css_class="col-6")),
            HTML(dynamiccauses),
...
class add_problem_form(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper = FormHelper()
        self.helper.form_tag = False
        self.helper.layout = Layout("categorie", "sous_categorie")
...
class updateDBForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(Row(Column("fichier")))
...
class installation_form(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(Row(Column("schema_installation")))
...
class Stattableauform(forms.Form):
    ...
    probleme = forms.MultipleChoiceField(
    ...
    cause = forms.MultipleChoiceField(
        widget=forms.CheckboxSelectMultiple,
    ...
    fichier_joint = forms.BooleanField(
        required=False, help_text="Cocher si fichier relié au ticket"
    )
    ...
    annee = forms.MultipleChoiceField(
        widget=forms.CheckboxSelectMultiple,
        choices=(("sans", "sans année"),)
        + tuple([(i, i) for i in range(2015, timezone.now().year + 1, 1)]),
        required=False,
    ...
    article_BL = forms.MultipleChoiceField(
        widget=forms.CheckboxSelectMultiple,
        required=False,
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        BL_with_tickets = list(
            ticket.objects.filter(BL__isnull=False).values_list("BL__BL", flat=True)
        )
        articles = (
        ...
        self.helper.layout = Layout(
            Row(
                Column(FloatingField("date_start"), css_class="col-6"),
                Column(FloatingField("date_end"), css_class="col-6"),
            ),
            Row(
                Column(
                    TabHolder(
                        Tab(
                            ...
                            Field(
                                "probleme",
                                size=10,
                                template="widgets/multiselect3colonne.html",
                            ),
                            HTML(
                                '<button type="button" id="button_problem" class="btn btn-outline-primary" onclick="Actionbouton_problem(this)">Tout désélectionner</button>'
                            ),
                        ),
                        Tab(
                            "Causes",
...
class classification_form(ModelForm):
    ...
    autre = forms.CharField(
        max_length=100, label="Autre titre de procédure", required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["titre"] = forms.ChoiceField(
            ...
            choices=[
                (c["id"], c["titre"])
                for c in classification.objects.all().values("titre", "id")
            ]
            + [("0", "Autre")],
        )
        self.initial = {"titre": self.instance.id if self.instance.pk != None else 0}
        self.helper = FormHelper()
        self.helper.form_tag = False
        self.helper.layout = Layout(
            Row(
...
class CL_Form(ModelForm):
    ...
    def __init__(self, show=None, *args, **kwargs):
        ...
        self.fields["prix_transport"].widget = NumberInput(
            attrs={"min": "0", "max": "10000", "step": "0.01"}
        )
        ...
        if self.instance.pk == None:
            ...
            if self.instance.date_prepa_carte or not self.instance.json():
                ...
                html = (
                    '<a href="/cartcreator/'
                    + str(self.instance.pk)
                    + '" class="btn btn-outline-primary" target="_blank">Créer carte</a>'
                )
