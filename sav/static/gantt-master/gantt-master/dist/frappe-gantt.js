    ...
    class Gantt {
        ...
        make_grid_highlights() {
            ...
            if (this.view_is(VIEW_MODE.DAY)) {
                //highlight weekend and holidayfrance
            var holiday = Object.keys(holidayFrance)
            var holiday2 = holiday.map( dateString => new Date(dateString) )
            var holiday3 = holiday2.map( d => new Date(d.setDate(d.getDate() - 1)).toISOString().split('T')[0] )
            for (var d = new Date(this.gantt_start); d <= new Date(this.gantt_end); d.setDate(d.getDate() + 1)){
                if(d.getDay() == 6 || d.getDay() == 0 || holiday3.includes(d.toISOString().split('T')[0])) {
                    const x =
